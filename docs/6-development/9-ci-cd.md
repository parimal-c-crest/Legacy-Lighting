...
| CI/CD Platform | GitHub Actions (test/lint/typecheck) + Vercel/Render native Git integration (build/deploy) |
| Repository | `legacy-lighting-p360` (GitHub) |
| Artifact Repository | None — Vercel/Render build directly from the Git repository, no separate artifact store |
| Deployment Strategy | Continuous Delivery to staging; manual promotion to production at Milestone 5 launch |
| Version | 1.0 |
| Status | Draft |
...
GitHub Actions runs lint/typecheck/unit/integration tests on every push and pull request. Once
checks pass, Vercel and Render's native GitHub integrations handle the actual build and deploy
independently — a push to `staging` deploys to the staging environment, a merge to `main`
deploys to production. No separate deployment step lives in GitHub Actions; it's a quality gate
only, not the deployer.
