...
| Role | Description |
|------|-------------|
| Admin | Full system access; user/role and master-data management |
| Manager | Reviews all projects/tasks/requests; dashboards, reports, reassignment |
| Processor | Manages request intake and task assignment |
| Estimator | Executes assigned tasks |
| Viewer | Read-only dashboard/report/project access |
...
# 8. Core Platform Flows

## Request Intake → Task Conversion (FR-INTAKE-004)
...
Create/Select Request
   ↓
Validate (Customer, Request Type, Due Date, Priority)
   ↓
Save (or Save Draft)
   ↓
Convert to Task
   ↓
Task appears in Task Workbench, estimator notified
```

## Task Execution (Estimator)

```
Task Workbench (List/Board)
   ↓
Open Task Card
   ↓
Update Status / Add Note / Mark Awaiting Info / Extend Due Date
   ↓
Mark Complete (confirmation required)
```

## Project 360 Review (Manager)
...
Select Project
   ↓
Project 360 Detail (tasks, requests, blockers, activity, references)
   ↓
Reassign Task / Update Priority (optional)
```

## Generic CRUD Flow

Applicable to master-data screens under Settings.
...
Settings
     ↓
Users / Master Data / Connected Systems
```
...
Executive Dashboard
     ↓
Projects 360 / Reports
     ↓
Reassign Task / Review Blocker
```
