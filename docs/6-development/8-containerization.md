...
| Container Platform | None — not used anywhere in this project (confirmed 2026-07-27) |
| Orchestration Platform | None |
| Container Registry | None — Vercel/Render build and deploy directly from source, no container images published |
| Version | 1.0 |
| Status | Draft |
...
**Not applicable to this project.** No Docker or any other containerization is used anywhere —
confirmed 2026-07-27. Local PostgreSQL runs as a natively installed service on each developer's
machine (see `1-development-environment.md` Section 10), and staging/production run on Vercel
(frontend) and Render (backend + managed Postgres), neither of which requires a container image
to be built or published. This keeps operational overhead near zero for a 2-developer team with
no dedicated DevOps role. The rest of this document is retained as a template only, in case
container use is revisited in a future phase.
